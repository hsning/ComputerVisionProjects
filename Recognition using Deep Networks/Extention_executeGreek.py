...
greek_train = torch.utils.data.DataLoader(
    datasets.ImageFolder( "greek_train-6",
...
def main(argv):
    # main function code
    labels=["alpha","beta","gamma", "lambda","mu","theta"]
    model = MyNetwork().to("cpu")
    model.load_state_dict(torch.load("result/model.pth"))
    loss_fn = nn.CrossEntropyLoss()
    for param in model.parameters():
        param.requires_grad = False
    model.conv1=nn.Conv2d(in_channels=1, out_channels=10, kernel_size=(5,5))
    model.conv2=nn.Conv2d(in_channels=10, out_channels=20, kernel_size=(5,5))
    model.mul_factor=4
    model.fc1 = nn.Linear(in_features=80,out_features=20)
    model.fc2=nn.Linear(in_features=20,out_features=6)
    ...
    #data_transform = transforms.Compose([transforms.RandomInvert(1),transforms.Resize((28,28)),transforms.Grayscale(num_output_channels=1),
                                     #transforms.ToTensor()])
    test_data = datasets.ImageFolder('testDataGreek-6/', transform = transforms.Compose( [transforms.ToTensor(),                                                             
                          GreekTransform(),
                          transforms.Normalize(
                          (0.1307,), (0.3081,) ) ] ))
