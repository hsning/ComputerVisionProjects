...
def main(argv):
    numberOfFilters = [1/10, 1/5, 1/2, 1, 6/5, 3/2, 9/5, 2, 5/2, 3, 4, 5]
    ...
    for j in range(0, len(numberOfFilters)):
        modelA = MyNetwork().to("cpu")
        modelA.conv1=nn.Conv2d(in_channels=1, out_channels=(int)(10*numberOfFilters[j]), kernel_size=(5,5))
        modelA.conv2=nn.Conv2d(in_channels=(int)(10*numberOfFilters[j]), out_channels=(int)(20*numberOfFilters[j]), kernel_size=(5,5))
        modelA.fc1=nn.Linear(in_features=(int)(320*numberOfFilters[j]),out_features=(int)(50))
        modelA.fc2=nn.Linear(in_features=(int)(50),out_features=10)
        print("Increase # of filters by a factor of: "+str(numberOfFilters[j]))
