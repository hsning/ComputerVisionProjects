...
int segement( cv::Mat &src, cv::Mat &dst, std::vector<std::vector<float>> &regionsMatrix, std::map<int, int> regions);
int computeMomentsFeautures( cv::Mat &regionMap, int regionID, cv::Moments &m, cv::RotatedRect &boundingRect, float &orientedCentralMoments);
int extractCorners( cv::Mat &src,std::vector< cv::Point2f> &corners);